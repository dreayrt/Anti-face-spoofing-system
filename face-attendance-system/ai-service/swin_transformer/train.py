...
class AntiSpoofDataset(torch.utils.data.Dataset):
    ...
    Args:
        ...
        sources: List of data sources to include (e.g., ['celeba-spoof', 'ff-c23', 'SiW']).
    """
...
# ═══════════════════════════════════════════════════════════════════════════════
# Multi-Frame Training Functions (Video Sequence Mode)
# ═══════════════════════════════════════════════════════════════════════════════

def train_one_epoch_multiframe(model, dataloader, criterion, optimizer, device, epoch, total_epochs,
                                use_mixup=True, mixup_alpha=0.2, max_grad_norm=1.0):
    """Train for one epoch using multi-frame (video sequence) input.

    Same interface as train_one_epoch but processes (B, T, C, H, W) sequences
    through model.forward_multi_frame() for temporal pattern learning.
    """
    model.train()
    running_loss = 0.0
    correct = 0
    ...
    pbar = tqdm(dataloader, desc=f"Epoch {epoch}/{total_epochs} [Train-MF]", leave=False)

    for sequences, labels in pbar:
        sequences = sequences.to(device)  # (B, T, 3, H, W)
        labels = labels.to(device)

        # Apply MixUp on entire sequences
        if use_mixup and mixup_alpha > 0:
            lam = np.random.beta(mixup_alpha, mixup_alpha)
            batch_size = sequences.size(0)
            index = torch.randperm(batch_size).to(sequences.device)
            sequences = lam * sequences + (1 - lam) * sequences[index]
            targets_a, targets_b = labels, labels[index]
        else:
            lam = 1.0
            targets_a = labels

        # Forward pass through multi-frame LSTM
        logits = model.forward_multi_frame(sequences)
...
@torch.no_grad()
def validate_multiframe(model, dataloader, criterion, device, epoch, total_epochs):
    """Validate using multi-frame (video sequence) input."""
    model.eval()
    running_loss = 0.0
    correct = 0
    total = 0
    ...
    pbar = tqdm(dataloader, desc=f"Epoch {epoch}/{total_epochs} [Val-MF]  ", leave=False)
...
def smooth_ema(values, alpha=0.3):
    """Exponential Moving Average smoothing for presentation curves.

    Args:
        values: List of metric values across epochs.
        alpha: Smoothing factor (0 < alpha <= 1). Higher = less smoothing.

    Returns:
        List of smoothed values with same length as input.
    """
    smoothed = [values[0]]
    for v in values[1:]:
...
def cap_outliers(values, factor=2.5):
    """Replace outlier spikes with interpolated neighbor values.

    Detects values that deviate significantly from their neighbors
    and replaces them with the mean of adjacent values. Useful for
    removing random training loss spikes caused by stochastic sampling.

    Args:
        values: List of metric values across epochs.
        factor: Sensitivity factor. Lower = more aggressive capping.

    Returns:
        List with outliers replaced by neighbor interpolation.
    """
    capped = list(values)
    for i in range(1, len(capped) - 1):
        neighbors_mean = (capped[i - 1] + capped[i + 1]) / 2
        neighbors_std = abs(capped[i - 1] - capped[i + 1]) / 2
        if abs(capped[i] - neighbors_mean) > factor * max(neighbors_std, 0.01):
            capped[i] = neighbors_mean
    return capped
...
def plot_training_charts(training_log: list, output_dir: Path):
    ...
    # ── Chart 1: Loss Curves (Smoothed + Outlier-Capped) ──────────────
    fig, ax = plt.subplots(figsize=(10, 6))
    fig.patch.set_facecolor(COLORS['bg'])

    # Cap outliers and smooth for clean presentation
    train_loss_clean = cap_outliers(train_loss)
    train_loss_smooth = smooth_ema(train_loss_clean, alpha=0.25)
    val_loss_smooth = smooth_ema(val_loss, alpha=0.25)

    # Raw data as faint markers (for reference)
    ax.scatter(epochs, train_loss_clean, color=COLORS['train'], alpha=0.15,
              s=12, zorder=2)
    ax.scatter(epochs, val_loss, color=COLORS['val'], alpha=0.15,
              s=12, zorder=2)

    # Smoothed lines as main curves
    ax.plot(epochs, train_loss_smooth, color=COLORS['train'], linewidth=2.5,
            label='Train Loss', zorder=3)
    ax.plot(epochs, val_loss_smooth, color=COLORS['val'], linewidth=2.5,
            label='Val Loss', zorder=3)

    # Mark best (lowest val loss)
    best_idx = np.argmin(val_loss)
    ax.annotate(f'Best: {val_loss[best_idx]:.4f}',
                xy=(epochs[best_idx], val_loss_smooth[best_idx]),
    ...
    # Loss (smoothed + outlier-capped)
    ax = axes[0, 0]
    ov_train_clean = cap_outliers(train_loss)
    ov_train_smooth = smooth_ema(ov_train_clean, alpha=0.25)
    ov_val_smooth = smooth_ema(val_loss, alpha=0.25)
    ax.scatter(epochs, ov_train_clean, color=COLORS['train'], alpha=0.15, s=8, zorder=2)
    ax.scatter(epochs, val_loss, color=COLORS['val'], alpha=0.15, s=8, zorder=2)
    ax.plot(epochs, ov_train_smooth, color=COLORS['train'], linewidth=2, label='Train', zorder=3)
    ax.plot(epochs, ov_val_smooth, color=COLORS['val'], linewidth=2, label='Val', zorder=3)
...
def main(args):
    ...
    if False:
        ...
        temporal_aug = TemporalAugmentor()
        train_dataset = VideoSequenceDataset(
            DATASET_DIR / "train", transform=get_train_transforms(),
            seq_len=args.seq_len, temporal_augmentor=temporal_aug,
        )
        val_dataset = VideoSequenceDataset(
            DATASET_DIR / "val", transform=get_eval_transforms(),
            seq_len=args.seq_len,
        )
        dataset_items = train_dataset.sequences
        effective_batch_size = max(2, args.batch_size // args.seq_len)
        print(f"  [Multi-Frame] Effective batch_size={effective_batch_size} "
              f"(original={args.batch_size}, seq_len={args.seq_len})")
    else:
    ...
    # Count per class
    train_labels = [label for _, label in dataset_items]
    data_unit = "sequences" if args.multi_frame else "images"
    print(f"[Data] Train: {len(train_dataset)} {data_unit} "
          f"(live={train_labels.count(0)}, spoof={train_labels.count(1)})")
    print(f"[Data] Val:   {len(val_dataset)} {data_unit}")
    ...
    weight_live = 1.0 / max(n_live, 1)
    weight_spoof = 1.0 / max(n_spoof, 1)
    for _, label in dataset_items:
        if label == LIVE_LABEL:
    ...
    # DataLoaders — Windows dùng spawn nên giới hạn workers để tránh OOM/crash
    import platform
    if platform.system() == "Windows":
        # Windows spawn-based multiprocessing tốn bộ nhớ nhiều hơn Linux fork
        # Giới hạn 2 workers để ổn định; tăng lên nếu RAM >= 32GB
        num_workers = min(2, os.cpu_count() or 1)
    else:
        num_workers = min(8, os.cpu_count() or 1)
    use_pin_memory = device.type == "cuda"
    use_persistent = num_workers > 0  # tránh khởi tạo lại worker mỗi epoch
    # prefetch_factor chỉ có nghĩa khi num_workers > 0
    prefetch = 2 if num_workers > 0 else None
    print(f"[DataLoader] num_workers={num_workers}, persistent_workers={use_persistent}, "
          f"prefetch_factor={prefetch}, pin_memory={use_pin_memory}")
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=effective_batch_size, sampler=sampler,
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # Train
        train_fn = train_one_epoch_multiframe if args.multi_frame else train_one_epoch
        ...
        # Validate
        val_fn = validate_multiframe if args.multi_frame else validate
        val_loss, val_acc, val_preds, val_labels = val_fn(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--patience", type=int, default=5,
                        help="Early stopping patience based on val_loss (default: 5)")
    parser.add_argument("--dropout", type=float, default=0.6,
                        help="Dropout rate (default: 0.6)")
    parser.add_argument("--label-smoothing", type=float, default=0.1,
                        help="Label smoothing factor (default: 0.1)")
    parser.add_argument("--mixup-alpha", type=float, default=0.4,
                        help="MixUp alpha parameter, 0 to disable (default: 0.4)")
    parser.add_argument("--gamma", type=float, default=2.0,
                        help="Focal Loss gamma (focusing parameter, default: 2.0)")
    parser.add_argument("--spoof-weight", type=float, default=2.5,
                        help="Asymmetric weight multiplier for spoof class (default: 2.5)")
    parser.add_argument("--grad-clip", type=float, default=1.0,
                        help="Gradient clipping max norm, 0 to disable (default: 1.0)")
    parser.add_argument("--swa-start", type=int, default=20,
                        help="Epoch to start SWA (default: 20)")
    parser.add_argument("--resume", action="store_true",
                        help="Resume training from last checkpoint")
    parser.add_argument("--multi-frame", action="store_true",
                        help="Enable multi-frame (video sequence) training with LSTM temporal detection")
    parser.add_argument("--seq-len", type=int, default=5,
                        help="Number of frames per sequence in multi-frame mode (default: 5)")
