"""
test.py – Test/Evaluation script for CNN + DSP + LSTM Anti-Spoofing Model.
...
def plot_test_overview(metrics, labels, probs, preds, cm, output_dir, threshold=0.5):
    ...
    fig.patch.set_facecolor(COLORS['bg'])
    fig.suptitle('CNN + DSP + LSTM Anti-Spoofing — Test Evaluation Overview',
...
def main(args):
    print("=" * 70)
    print("  CNN + DSP + LSTM Anti-Spoofing Model — Test Evaluation")
    print("=" * 70)
    ...
    if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
        ...
        backbone_used = checkpoint.get("backbone", args.backbone)
    ...
    results = {
        ...
        "backbone": args.backbone,
        "eval_mode": "multi_frame" if args.multi_frame else "single_frame",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test/Evaluate CNN+DSP+LSTM Anti-Spoofing Model",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Ví dụ:
  # Single-frame (LSTM seq=1, không khai thác temporal)
  python test.py

  # Multi-frame (LSTM xử lý T frames — khuyến nghị)
  python test.py --multi-frame
  python test.py --multi-frame --seq-len 5
  python test.py --multi-frame --seq-len 8 --batch-size 16
"""
    ...
    parser.add_argument("--checkpoint", type=str,
                        default=str(WEIGHTS_DIR / "antispoof_cnn_dsp_lstm.pth"),
                        help="Path to model checkpoint (default: models/weights/antispoof_cnn_dsp_lstm.pth)")
    ...
    parser.add_argument("--multi-frame", action="store_true",
                        help="Bật multi-frame mode: LSTM xử lý T frames temporal sequence "
                             "thay vì seq_len=1. Khuyến nghị khi model train với --multi-frame.")
    parser.add_argument("--seq-len", type=int, default=5,
                        help="Số frame mỗi sequence trong multi-frame mode (default: 5). "
                             "Nên match với --seq-len dùng khi train.")
