"""
dataset_ffc23.py – PyTorch Dataset, DataLoader, và class-balancing cho FF-C23.

Cung cấp:
  - AntiSpoofDataset          (torch Dataset wrapper)
  - get_class_weights          (inverse-frequency weights cho loss)
  - get_weighted_sampler       (WeightedRandomSampler cho oversampling)
  - create_dataloaders         (factory trả về train/val/test loaders)
"""
...
class AntiSpoofDataset(Dataset):
    """Wrapper quanh ``ImageFolder`` để dễ truy vấn class counts."""
    ...
    def print_distribution(self, split_name: str = "") -> None:
        ...
        logger.info("%sPhân bố class (tổng=%d):", header, total)
...
def get_class_weights(dataset: AntiSpoofDataset) -> torch.Tensor:
    """Tính inverse-frequency class weights cho ``CrossEntropyLoss``."""
...
def get_weighted_sampler(dataset: AntiSpoofDataset) -> WeightedRandomSampler:
    ...
    logger.info(
        "WeightedRandomSampler – weights: %s",
...
def create_dataloaders(
    ...
    Returns  { split_name: (DataLoader, AntiSpoofDataset) }
    """
    ...
    for split_name in ("train", "val", "test"):
        split_path = data_dir / split_name / config.DATASET_NAME
        if not split_path.exists():
            logger.warning("Thư mục '%s' không tồn tại – bỏ qua.", split_path)
            continue
        ...
        logger.info(
            "[%s] DataLoader sẵn sàng – %d batches (batch_size=%d).",
