"""
dataset_siw.py – PyTorch Dataset, DataLoader, và class-balancing cho SiW.
...
from .augmentation_siw import get_eval_transforms, get_train_transforms
...
class AntiSpoofDataset(Dataset):
    """Wrapper quanh ``ImageFolder`` cho SiW dataset."""
...
def get_weighted_sampler(dataset: AntiSpoofDataset) -> WeightedRandomSampler:
    """Build ``WeightedRandomSampler`` để oversample minority class."""
    counts = dataset.class_counts()
    total = sum(counts.values())
    class_weights = {
    ...
    logger.info(
        "WeightedRandomSampler created - weights: %s",
...
def create_dataloaders(
    ...
    """Tạo train / val / test ``DataLoader`` cho SiW.

    Parameters
    ----------
    data_dir : Path
        Root dataset (chứa train/, val/, test/).
    batch_size : int
        Mini-batch size.
    num_workers : int
        Số workers.
    use_weighted_sampler : bool
        Nếu True, training loader dùng WeightedRandomSampler.
    ...
    for split_name in ("train", "val", "test"):
        ...
        if not split_path.exists():
            logger.warning("Directory '%s' does not exist - skipping.", split_path)
            continue
